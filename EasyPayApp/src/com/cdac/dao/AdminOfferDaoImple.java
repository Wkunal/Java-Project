	...
	public void insertAdminOffers(AdminAddOffers offers) {
		hibernateTemplate.execute(new HibernateCallback<Void>() {
			...
			public Void doInHibernate(org.hibernate.Session session) throws HibernateException {
				...
				session.save(offers);
				tr.commit();
				session.flush();  //Flushing the session will execute all the pending SQL statements, and when you clear the session, all the entities are evicted from the persistent context. 
				session.close();
				return null;
			}
		});
